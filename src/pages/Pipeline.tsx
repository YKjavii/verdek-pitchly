  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h1 className="text-2xl font-semibold mt-1">Every prospect, by stage</h1>
      </div>
              ...
              <div className="space-y-2">
                {col.map((p) => (
                  <Link
                    key={p.id}
                    to={`/prospects/${p.id}`}
                    className="block rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-3 text-sm hover:border-brand-400"
                  >
                    <div className="font-medium truncate">{p.name}</div>
                    <div className="text-xs text-neutral-500 mt-0.5">{p.area} · score {p._score}</div>
                  </Link>
                ))}
